...
This project does **not** generate licenses, patch `paul.dll`, or spoof `iphlpapi.dll`. It reproduces the runtime channel the game already uses, a named file mapping plus `SendMessage` IPC to a emulator window.

Do not run this with a real SecuROM binary. They will fight over the mapping name WWE (the rock) style.
...
The mapping is not a boolean “DRM was here.” It holds the HWND, message id, vendor/version fields, and the path/PID of the process that owns the window. The game then `SendMessage`s that HWND for the rest of the session and hashes a buffer the emulator writes back.
...
4. Answers `SendMessage` on `WM_APP + 0x51`. `lParam` pointers live in the **game**; the emulator uses `ReadProcessMemory` / `WriteProcessMemory`.
...
| `+0x04` | HWND | emulator window (low 32 bits) | `SendMessage` target |
...
| `+0x6C0` | cookie | emulator PID | must match parent PID |
